...
Terrain::Terrain()
    : GameObject(L"Landscape/Terrain.hlsl"), m_width(10), m_height(10)
{    
    m_material->SetDiffuseMap(L"Textures/Landscape/Dirt2.png");
    ...
    m_heightMap = Texture::Add(L"Textures/HeightMaps/AStarMap.png");
    m_alphaMap = Texture::Add(L"Textures/AlphaMaps/TestAlphaMap.png");
    m_secondMap = Texture::Add(L"Textures/Landscape/Dirt.png");
    m_thirdMap = Texture::Add(L"Textures/Landscape/Dirt3.png");
...
{
    int x = (int)pos.x;
    int z = (int)(m_height - pos.z - 1);

    if (x < 0 || x >= m_width - 1) return 0.0f;
    if (z < 0 || z >= m_height - 1) return 0.0f;

    UINT index[4];
    index[0] = m_width * z + x;
    index[1] = m_width * (z + 1) + x;
    index[2] = m_width * z + x + 1;
    index[3] = m_width * (z + 1) + x + 1;
...
Vector3 Terrain::Picking()
{
    ...
    for (UINT z = 0; z < m_height - 1; z++)
    {
        for (UINT x = 0; x < m_width - 1; x++)
        {
            UINT index[4];
            index[0] = m_width * z + x;
            index[1] = m_width * z + x + 1;
            index[2] = m_width * (z + 1) + x;
            index[3] = m_width * (z + 1) + x + 1;            
...
{
    m_width = (UINT)m_heightMap->GetSize().x;
    m_height = (UINT)m_heightMap->GetSize().y;

    vector<Float4> pixels;
    m_heightMap->ReadPixels(pixels);
    ...
    for (UINT z = 0; z < m_height; z++)
    {
        ...
        {
            VertexType vertex;
            vertex.pos = { (float)x, 0.0f, (float)(m_height - z - 1)};
            vertex.uv.x = x / (float)(m_width - 1);
            vertex.uv.y = z / (float)(m_height - 1);

            UINT index = m_width * z + x;
    ...
    for (UINT z = 0; z < m_height - 1; z++)
    {
        for (UINT x = 0; x < m_width - 1; x++)
        {
            indices.push_back(m_width * z + x);//0
            indices.push_back(m_width * z + x + 1);//1
            indices.push_back(m_width * (z + 1) + x);//2            

            indices.push_back(m_width * (z + 1) + x);//2
            indices.push_back(m_width * z + x + 1);//1            
            indices.push_back(m_width * (z + 1) + x + 1);//3
        }
    }    
}
