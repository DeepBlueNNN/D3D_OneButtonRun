...
GeometryInput VS(VertexUV input)
{
    ...
    alpha = saturate(1.5f + (alpha / (drawDistance * 20.0f)));
...
{
    float3 up = normalize((input[0].velocity * -1.0f));
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        pixelInput.color.a = 0.2f * saturate(1.0f - (pixelInput.pos.z / input[0].distance)) * input[0].color.a;
